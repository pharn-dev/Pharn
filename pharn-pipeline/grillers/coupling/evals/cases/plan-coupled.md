...
- `pharn-core/metrics-store.mjs` — a module-level MUTABLE singleton `state = {}` that the two modules below both import and write/read — layer pharn-core.
- `pharn-core/timer.mjs` — on request start, writes `state.startedAt = now()` into the shared singleton — layer pharn-core.
- `pharn-core/reporter.mjs` — later reads `state.startedAt` back from the shared singleton to compute and emit the duration — layer pharn-core.
