...
- `pharn-contracts/timing-shape.md` — the immutable `{ startedAt, durationMs }` shape both consumers depend on — layer pharn-contracts.
- `pharn-core/timer.mjs` — returns a fresh timing value; holds no shared state — layer pharn-core.
- `pharn-core/reporter.mjs` — receives a timing value as an explicit argument and emits it — layer pharn-core.
...
- The only thing both depend on is `pharn-contracts/timing-shape.md` (the bottom of the tree), never each other — clean seams.
