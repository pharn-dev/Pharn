...
**Advisory disposition:** does not block. The scanner is advisory and deliberately biased conservative-toward-CLEAN (under-flagging), so a weak-cleanup token fits that bias; `unref` was in the approved plan's method list. The honest refinement (P7 — on a real failure, not now): either drop `unref` from the set, or add a one-line bound stating it is a deliberate under-flag. Recorded for a future increment; **not** a reason to withhold this one.
