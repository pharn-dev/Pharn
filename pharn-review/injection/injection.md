...
This is the **injection twin of the `secrets-in-code` lens** (`pharn-review/secrets-in-code/`) and a sibling of
the `trust-fence` lens (`pharn-review/trust-fence/`, the P2 lens precedent): secrets-in-code catches a
...
(a lens never "decides approve" — `ARCHITECTURE.md §7`). When genuinely ambiguous, emit the finding and **ask
...
`features/injection/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission (the
...
- **Concat/interp-into-sink detection over CODE** (`.dev/floor/scan-code-injection.mjs`, a fixed regex set over
  the code text) → **FLOOR** (regex; `ARCHITECTURE.md §2` primitive #3), and **injection-immune by
