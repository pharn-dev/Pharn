...
This is a sibling of the `trust-fence` lens (`pharn-review/trust-fence/`, the P2 lens precedent) and shares the
honest posture of the `architecture-griller` (`pharn-pipeline/grillers/architecture/`, the "advisory-only beyond
...
(`ARCHITECTURE.md §2` primitive #3, enum/regex; fix #6 binding). A prose / code-block mention never registers.
...
human; you **never** gate on it (a lens never "decides approve" — `ARCHITECTURE.md §7`). This advisory-only
...
`features/race-condition/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission
