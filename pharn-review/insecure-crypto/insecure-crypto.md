...
This is a sibling of the `secrets-in-code` (`pharn-review/secrets-in-code/`) and `injection`
(`pharn-review/injection/`) lenses — the same **code-side scanner lens** shape, and the same P2 discipline as
the `trust-fence` lens (`pharn-review/trust-fence/`): secrets-in-code catches a hardcoded **secret literal**,
...
`ARCHITECTURE.md §7`), and you **never** suppress a scanner hit on that basis. When genuinely ambiguous, emit
...
`features/insecure-crypto/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md`
