...
**exactly one** finding, in the `pharn-contracts/finding-shape` object, with the split honored.
...
- type: FINDING # enum-gated (floor-verifiable) — fixed by the scanner's regex hit
  ...
  file: "pharn-review/unsafe-deserialization/evals/cases/case-safe-comment.md:15" # enum-gated — the eval line, FROM THE SCANNER
