...
the `pharn-contracts/finding-shape` object, with the enum-gated / free-text split honored.
...
- type: FINDING # enum-gated (floor-verifiable) — fixed by the scanner's regex hit
  ...
  file: "pharn-review/unsafe-deserialization/evals/cases/case-pickle-loads.md:17" # enum-gated — the call line, FROM THE SCANNER
