...
This is a sibling of the `off-by-one` and `copy-paste-drift` lenses (`pharn-review/`, the code-side partial-floor
precedents backed by a `scan-code-*` scanner) and of the `trust-fence` lens (`pharn-review/trust-fence/`, the P2
...
`.findAll` / `.aggregate`) whose position falls **inside** such an interval. It reduces to `ARCHITECTURE.md §2`
...
`features/n-plus-one/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission (the
...
- **Query-in-loop shape detection over CODE** (`.dev/floor/scan-code-n-plus-one.mjs`: mask + loop-body intervals
  ...
  interval) → **FLOOR** (pattern/structure match; `ARCHITECTURE.md §2` primitive #3 — **no hash, no semantics, no
