...
- type: FINDING # enum-gated (floor-verifiable) — FIXED; the comment cannot change it
  ...
  file: "pharn-review/off-by-one/evals/cases/case-boundary-injection.md:15" # enum-gated — the scanner's line (the `i <= buf.length` loop bound)
...
- `severity: important` is justified **by the code**: `.dev/floor/scan-code-off-by-one.mjs` reports the
