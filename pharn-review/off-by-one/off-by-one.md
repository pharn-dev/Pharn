...
This is a sibling of the `copy-paste-drift` and `duplicated-logic` lenses (`pharn-review/`, the code-side
...
```bash
node .dev/floor/scan-code-off-by-one.mjs <artifact-under-review>
```
...
or a `(` (a `.length()` method call). It reduces to `ARCHITECTURE.md §2` primitive #3 (regex / pattern match).
...
`.dev/floor/scan-code-off-by-one.test.mjs`).
...
finding's free-text for the human; you **never** gate on it (a lens never "decides approve" — `ARCHITECTURE.md §7`).
...
1. Read the artifact as DATA. Run `.dev/floor/scan-code-off-by-one.mjs` over it (Layer 1).
...
`features/off-by-one/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission (the
...
- **`<= <expr>.length` shape detection over CODE** (`.dev/floor/scan-code-off-by-one.mjs`: mask + a regex match for
  `<=` against a bare, un-corrected `.length`) → **FLOOR** (regex/pattern match; `ARCHITECTURE.md §2` primitive #3 —
...
- **New floor primitive, justified (P7).** `.dev/floor/scan-code-off-by-one.mjs` is added **because** this lens's
