...
file** under review — and emit zero or more findings in the `pharn-contracts/finding-shape` object. You
**cite** the principle you enforce (`P2`); you do not restate it (P4). You **cannot** "decide approve" —
you emit a typed finding list or nothing (`ARCHITECTURE.md §7`).

This is the **code-side twin of the security griller** (`pharn-pipeline/grillers/security/`, which scans
a **PLAN**) and a sibling of the `trust-fence` lens (`pharn-review/trust-fence/`, the P2 lens precedent):
...
assigned a quoted literal), reducing to `ARCHITECTURE.md §2` primitive #3. **For each hit, emit one
...
**never** gate on it (a lens never "decides approve" — `ARCHITECTURE.md §7`). When genuinely ambiguous,
...
`features/secrets-in-code/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md`
...
- **Secret-literal detection over CODE** (`.dev/floor/scan-code-secrets.mjs`, a fixed regex set over the
  code text) → **FLOOR** (regex; `ARCHITECTURE.md §2` primitive #3), and **injection-immune by
