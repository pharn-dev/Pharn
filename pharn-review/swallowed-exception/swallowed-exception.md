...
This is a sibling of the `injection` lens (`pharn-review/injection/`, the code-side partial-floor precedent) and of
the `trust-fence` lens (`pharn-review/trust-fence/`, the P2 lens precedent). Where `injection` catches a
...
human; you **never** gate on it (a lens never "decides approve" — `ARCHITECTURE.md §7`). When genuinely ambiguous,
...
`features/swallowed-exception/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission
...
- **Empty/log-only catch detection over CODE** (`.dev/floor/scan-code-swallowed-exception.mjs`: mask + catch regex +
  brace-match + first-match classification) → **FLOOR** (regex/text membership; `ARCHITECTURE.md §2` primitive #3),
