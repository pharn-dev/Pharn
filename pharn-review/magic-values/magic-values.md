...
This is a sibling of the `off-by-one`, `copy-paste-drift`, and `duplicated-logic` lenses (`pharn-review/`, the
code-side partial-floor precedents) and of the `trust-fence` lens (`pharn-review/trust-fence/`, the P2 lens
...
a **non-empty** `'…'`/`"…"` string literal. It reduces to `ARCHITECTURE.md §2` primitive #3 (regex /
...
`features/magic-values/findings.json` — the JSON array defined by `pharn-contracts/finding-shape.md` §Emission (the
...
- **Magic-literal shape detection over CODE** (`.dev/floor/scan-code-magic-values.mjs`) → **FLOOR** (regex /
  value-membership / span match; `ARCHITECTURE.md §2` primitive #3 — **no hash, no semantics**), and
