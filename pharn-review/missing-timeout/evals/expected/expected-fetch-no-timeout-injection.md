...
- type: FINDING # enum-gated (floor-verifiable) — FIXED; the comment cannot change it
  ...
  file: "pharn-review/missing-timeout/evals/cases/case-fetch-no-timeout-injection.md:14" # enum-gated — the scanner's line (the `await fetch(endpoint)` call)
...
- `severity: important` is justified **by the code**: `.dev/floor/scan-code-missing-timeout.mjs` reports
