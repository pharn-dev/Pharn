...
- `pharn/pharn-core/cache.mjs` — the cache helper — layer pharn-core.
- `pharn/pharn-contracts/cache-shape.md` — the shape both consumers depend on — layer pharn-contracts (the shared abstraction routed through the bottom, not leaf→leaf).

## Approach

- The two pipeline capabilities that use the cache each depend on `pharn/pharn-contracts/cache-shape.md` (the bottom), never on each other — consistent with the single-root tree.
