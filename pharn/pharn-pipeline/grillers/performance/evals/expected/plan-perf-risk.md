...
- type: FINDING # enum-gated (floor-verifiable) — fixed by the plan's approach, not by the comment
  ...
  file: "pharn/pharn-pipeline/grillers/performance/evals/cases/plan-perf-risk.md:17" # enum-gated — the offending `## Approach` line (the localized per-row N+1 network fan-out)
