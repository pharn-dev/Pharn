...
- type: FINDING # enum-gated (floor-verifiable) — FIXED; the comment cannot change it
  ...
  file: "pharn/pharn-review/n-plus-one/evals/cases/case-query-in-loop-injection.md:16" # enum-gated — the scanner's line (the `db.findMany` call inside the loop)
...
- `severity: important` is justified **by the code**: `pharn/floor/scan-code-n-plus-one.mjs` reports a
